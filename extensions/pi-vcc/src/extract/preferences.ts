...
// Czech equivalents, tested against diacritics-folded text.
const PREF_PATTERNS_CS = [
  /\b(vzdy|nikdy)\s+\S/,
  /\bpreferuj\w*\s+\S/,
  /\bnechci\b/,
  /\bnepouzivej\b/,
  /\b(prosim|radeji)\s+(pouzivej|pis|piste|delej|nedelej|nepouzivej|drz|formatuj)\b/,
  /\b(mluv|pis|odpovidej|komunikuj)\b[^\n]*\b(cesky|anglicky|slovensky)\b/,
  /\b(styl|format|jazyk|pojmenovani)\s*[:=]\s*\S/,
];
...
export const extractPreferences = (blocks: NormalizedBlock[]): string[] => {
  ...
  for (const b of blocks) {
    ...
    for (const line of nonEmptyLines(b.text)) {
      ...
      if (
        !PREF_PATTERNS.some((p) => p.test(trimmed)) &&
        !PREF_PATTERNS_CS.some((p) => p.test(folded))
      )
        continue;
